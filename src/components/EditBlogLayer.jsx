...
import { updateBlog, getAllBlogs } from "../redux/slice/blogSlice";
...
const EditBlogLayer = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      dispatch(updateBlog({id: blogId, data})).unwrap();
...
useEffect(() => {
  if (blogById!==null) {
    ...
    excerpt: blogById.excerpt || "",
    content: blogById.content || "",
    tags:  blogById.tags || [],
    image: blogById.image || null,
    });

    // Set preview to the image URL from backend
    if (blogById.image) {
      setImagePreview(blogById.image); // use backend image URL
      setImageDimensions("1000 x 707"); // optional: show recommended dimensions
      setImageError(""); // no error
    }
  }
}, [blogById]);
